...
const room = {
    ...
    maxFood: c.WIDTH * c.HEIGHT / 19500 * c.FOOD_AMOUNT,
    isInRoom: location => {
    ...
    room.findType('roid');
    room.findType('rock');
    room.nestFoodAmount = 1.45 * Math.sqrt(room.nest.length) / room.xgrid / room.ygrid;
    room.random = () => {
        return {
...
class Gun {
    ...
    interpret() {
        ...
        // Defaults
        let out = {
            SPEED: shoot.maxSpeed * sk.spd * (0.5 + sizeFactor / 5),
            HEALTH: shoot.health * sk.str * (sizeFactor / 1.65),
            RESIST: shoot.resist + sk.rst,
            DAMAGE: shoot.damage * sk.dam * (sizeFactor / 1.65),
        ...
        case 'drone':
            out.PUSHABILITY = 1;
            out.PENETRATION = Math.max(1, shoot.pen * (0.5 * (sk.pen - 1) + 1));
            out.HEALTH = (shoot.health * sk.str + Math.sqrt(sizeFactor) / Math.pow(sk.pen, 0.8));
...
class Entity {
    ...
    refreshBodyAttributes() {
        ...
        this.health.set(
            (((this.settings.healthWithLevel) ? 2.05 * this.skill.level : 0) + this.HEALTH) * this.skill.hlt
        );

        this.health.resist = 1.05 - 1 / Math.max(1, this.RESIST + this.skill.brst);

        this.shield.set(
            (((this.settings.healthWithLevel) ? 0.7 * this.skill.level : 0) + this.SHIELD) * this.skill.shi, 
            Math.max(0, ((((this.settings.healthWithLevel) ? 0.00725 * this.skill.level : 0) + 1.05) * this.REGEN) * this.skill.rgn)
        );
        
        this.damage = (this.DAMAGE * this.skill.atk * (1 + this.skill.level / 50));
        ...
        this.fov = this.FOV * 255 * Math.sqrt(this.size) * (1 + 0.005 * this.skill.level);
        
        this.density = (1 + 0.1 * this.skill.level) * this.DENSITY; 
    ...
    get size() {
        if (this.bond == null) return (this.coreSize || this.SIZE) * (1 + this.skill.level / 36);
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case 'L': { // level up cheat
                    ...
                    // cheatingbois
                    if (player.body != null) { if (player.body.skill.level < c.SKILL_CHEAT_CAP || ((socket.key === process.env.SECRET) && player.body.skill.level < 500)) {
    ...
    let makefood = (() => {
        ...
        function getFoodClass(level) {
            ...
            switch (level) {
                ...
                case 7: a = Class.hyperhexagon; break;
        ...
        let makeNestFood = () => { // Make nest pentagons
         ...
         let makeHexFood = () => { // Make nest hexagons
            let spot = room.randomType('nest');
            placeNewFood(spot, 0.005 * room.width, 4, true);
        };
         let makeHyperHexFood = () => { // Make hyper nest hexagons
            let spot = room.randomType('nest');
            placeNewFood(spot, 0.003 * room.width, 7, true);
        };
        ...
        return () => {
            ...
            while (ran.chance(0.8 * (1 - foodAmount * foodAmount / maxFood / maxFood))) {
                switch (ran.chooseChance(10, 4, 2, 1, 0.1)) {
                case 0: makeGroupedFood(); break;
                case 1: makeDistributedFood(); break;
                case 2: makeCornerFood(); break;
                case 3: makeHexFood(); break;
                case 4: makeHyperHexFood(); break;
                }
            } 
